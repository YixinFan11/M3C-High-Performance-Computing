...
subroutine nnmodel_mpi(comm,fvec,n,m,d,c,cgrad)
  implicit none
  integer, intent(in) :: comm
  integer, intent(in) :: n,m,d !training data and inner layer sizes, !d is dlocal in this instance
...
subroutine fgd_mpi(comm,fvec_guess,n,m,dlocal,d,alpha,fvec)
  implicit none
  integer, intent(in) :: comm,n,m,dlocal,d  						!d is actually dtrain
  real(kind=8), dimension(:), intent(in) :: fvec_guess
  real(kind=8), intent(in) :: alpha									        !learning rate
  real(kind=8), dimension(size(fvec_guess)), intent(out) :: fvec	!final output
  integer :: i1, j1, i1max=1000, n_fvec, numprocs
  real(kind=8) :: c, c_total
  real(kind=8), dimension(size(fvec_guess)) :: cgrad,cgrad2
  integer :: myid,ierr
  ...
  do i1=1,i1max
    call nnmodel_mpi(comm,fvec,n,m,dlocal,c,cgrad2)

    call MPI_BARRIER(comm,ierr)  !Wait for all threads to be syncronised

    !Sending average of individual thread's cgrad to all other threads
    call MPI_ALLREDUCE(cgrad2,cgrad,size(fvec_guess),MPI_DOUBLE_PRECISION,MPI_SUM,comm,ierr)

    !Reducing c to calculate total cost across all processors
    call MPI_REDUCE(c,c_total,1,MPI_DOUBLE_PRECISION,MPI_SUM,0,comm,ierr)

    fvec = fvec-alpha*cgrad/dble(numprocs) !update fvec

    if ((myid==0) .and.(mod(i1,50)==0)) then
      c_total = c_total/dble(numprocs)
      print *, 'Completed epoch #', i1, 'cost on process 0 =',c
