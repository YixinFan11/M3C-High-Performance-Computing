...
program p1main_mpi
  ...
  integer :: m=4
  integer :: i1,j1,dtrain=4000, p(n+1,d)
  ...
  if (myid==0) print *, 'Number of processes is =', numprocs
  ...
  call MPI_BCAST(fvec0, (m*(n+2)+1), MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr) !Sending fvec0 to every processor

  call fgd_mpi(MPI_COMM_WORLD,fvec0,n,m,dlocal,dtrain,2.5d0,fvec)
